...
class EmbyMediaSource @Inject constructor(
    ...
    override suspend fun resolvePlayback(itemId: String): PlaybackItem {
        ...
        val streamUrl = browseRepository.getStreamUrl(itemId, preference?.mediaSourceId)
            ?: error("No Emby stream URL for: $itemId")
        ...
        // picker existed. Mirrors JellyfinMediaSource.resolvePlayback exactly.
    ...
    // back to PlaybackItem's own default (subtitlesOff = false) instead of a real decision. Mirrors
    // JellyfinMediaSource.resolveSubtitlePreference exactly.
    ...
    // forced and a full track can share the same language (common on e.g. anime releases). Mirrors
    // JellyfinMediaSource's private resolveSubtitlePreference overload exactly.
    private fun resolveSubtitlePreference(
        item: EmbyItemInfo,
        settings: EmbyAppSettings,
        preference: EmbyPlaybackPreference?,
    ): SubtitlePreference {
    ...
    private fun EmbyItemInfo.toPlaybackItem(
        streamUri: String,
