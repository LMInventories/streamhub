...
// Shared across phone and TV rather than split - same reasoning as EmbyLibraryScreen: a top bar +
// filter toggle + poster grid needs no orientation- or D-pad-specific structure of its own, so one
// implementation serves both nav hosts. Direct port of JellyfinFavoritesScreen, which is also a
// single shared file rather than having its own ...Tv sibling.
...
fun EmbyFavoritesScreen(
    ...
    viewModel: EmbyFavoritesViewModel = hiltViewModel(),
) {
    ...
    val filteredItems = uiState.items.filter { item ->
        if (modeIndex == 0) item.type == EmbyItemType.MOVIE else item.type != EmbyItemType.MOVIE
    }
